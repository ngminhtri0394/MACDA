...
def get_valid_actions_prot(state, scanning_type):
    valid_actions = set()
    for idx, residue in enumerate(state):
        if residue is not scanning_type:
            new_state = list(state)
            new_state[idx] = scanning_type
            valid_actions.add(''.join(new_state))
    return valid_actions


def get_valid_actions_drug(state, atom_types, allow_removal, allow_no_modification,
...
class DTAEnvs(object):
    ...
    def __init__(self,
        ...
        self._valid_actions_drug = []
        self._valid_actions_prot = []
        ...
        self._path = []
        self._max_bonds = 4
        self.agents = [Agent('drug'), Agent('protein')]
    ...
    def get_valid_actions_drug(self, state=None, force_rebuild=False):
        ...
        """
        if state is None:
            if self._valid_actions_drug and not force_rebuild:
                return copy.deepcopy(self._valid_actions_drug)
            state = self._state[0]
        if isinstance(state, Chem.Mol):
            state = Chem.MolToSmiles(state)
        self._valid_actions_drug = get_valid_actions_drug(
            state,
    ...
    def get_valid_actions_prot(self, state=None, force_rebuild=False):
        """Gets the valid actions for the state.
        ...
        """
        if state is None:
            if self._valid_actions_prot and not force_rebuild:
                return copy.deepcopy(self._valid_actions_prot)
            state = self._state[1]
        self._valid_actions_prot = get_valid_actions_prot(
            state,
            scanning_type='A')
        return copy.deepcopy(self._valid_actions_prot)
    ...
    def step(self, action):
        ...
        """
        assert len(action[0]) == len(self._valid_actions_drug), "Drug action space does not match"
        assert len(action[1]) == len(self._valid_actions_prot), "Protein action space does not match"
        drug_action = list(self._valid_actions_drug)[np.argmax(action[0])]
        prot_action = list(self._valid_actions_prot)[np.argmax(action[1])]
